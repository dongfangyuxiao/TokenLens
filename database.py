...
def init_db():
    with get_conn() as conn:
        ...
        conn.execute("CREATE INDEX IF NOT EXISTS idx_license_records_status ON license_records(record_status)")
        conn.execute("CREATE INDEX IF NOT EXISTS idx_license_records_customer ON license_records(customer)")
        conn.execute("CREATE INDEX IF NOT EXISTS idx_license_records_machine_code ON license_records(machine_code)")
        conn.execute("CREATE INDEX IF NOT EXISTS idx_license_records_created_at ON license_records(created_at)")
        ...
        conn.execute("INSERT OR IGNORE INTO app_config VALUES ('license_token','')")
        conn.execute("INSERT OR IGNORE INTO app_config VALUES ('license_enforce_enabled','0')")
        legacy_license = conn.execute("SELECT value FROM app_config WHERE key='license_key'").fetchone()
        current_license = conn.execute("SELECT value FROM app_config WHERE key='license_token'").fetchone()
        if legacy_license and legacy_license['value'] and (not current_license or not current_license['value']):
            conn.execute("UPDATE app_config SET value=? WHERE key='license_token'", (legacy_license['value'],))
...
def _normalize_license_record(row):
    ...
    status = item.get('record_status') or 'active'
...
def create_license_record(customer: str, product: str, machine_code: str, expires_at: str,
                          features: list, metadata: dict, license_token: str,
                          created_by: str = '', remark: str = ''):
    now = datetime.now().isoformat()
    with get_conn() as conn:
        cur = conn.execute(
...
def list_license_records(status: str = '', keyword: str = '', limit: int = 200):
    sql = (
        "SELECT id, customer, product, machine_code, expires_at, features, metadata, "
        "license_token, record_status, remark, created_by, created_at, updated_at, download_count, last_downloaded_at "
        "FROM license_records WHERE 1=1"
    )
    params = []
...
def mark_license_record_downloaded(record_id: int):
    ...
    with get_conn() as conn:
        conn.execute(
            ...
            "SET download_count=COALESCE(download_count,0)+1, last_downloaded_at=?, updated_at=? "
            "WHERE id=?",
            (now, now, record_id)
        )
        conn.commit()
